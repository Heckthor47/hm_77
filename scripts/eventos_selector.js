...
function configurarSelectorEstado() {
  ...
  selector.addEventListener("selection-change", (e) => {
    ...
    fetch(ruta)
      ...
      .then((data) => {
        // usar todas las features sin muestreo
        const finalFeatures = data.features;

        // actualizar la fuente con todos los datos
        if (map.getSource("calles")) {
          map.getSource("calles").setData({
        ...
        const vis = document.getElementById("toggle-calles").checked
          ? "visible"
          : "none";
        if (map.getLayer("calles-layer")) {
