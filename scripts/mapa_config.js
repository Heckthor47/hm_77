...
map.on("load", () => {
  // 1) Carga de entidades GeoJSON. Toda la configuración posterior depende de que esto termine.
  fetch("https://fabulous-dodol-d03b96.netlify.app/entidades.geojson")
    .then(r => r.json())
    .then(data => {
      // --- CAMBIO: INICIO ---
      // Toda la lógica de configuración de capas y eventos se mueve aquí adentro.
      // Esto garantiza que el código no intente usar la variable 'estadosGeoJson'
      // o manipular capas antes de que los datos esenciales estén listos.
      ...
      // 6) Finalmente, se inicia la función del selector de estados
